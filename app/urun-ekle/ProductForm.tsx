...
export default function ProductForm({ subcategories }: { subcategories: SubcategoryOption[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subcategorySlug, brand, model }) });
    setLoading(false);
    if (!res.ok) { const data = await res.json().catch(() => ({})); setError(data.error || "Ürün eklenemedi."); return; }
