...
export default async function AdminPage() {
  ...
  const products = await prisma.product.findMany({ include: { _count: { select: { reviews: true } } }, orderBy: { createdAt: "desc" } });
  const reviews = await prisma.review.findMany({ include: { user: true, product: true }, orderBy: { createdAt: "desc" }, take: 50 });
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      ...
      <p className="mt-2 mb-8 text-sm text-steel">Uygunsuz ürün veya yorumları buradan kaldırabilirsin.</p>
      <AdminList
        products={products.map((p) => ({ id: p.id, brand: p.brand, model: p.model, slug: p.slug, reviewCount: p._count.reviews }))}
        reviews={reviews.map((r) => ({ id: r.id, title: r.title, body: r.body, productModel: r.product.model, userDisplayName: r.user.displayName, createdAt: r.createdAt.toISOString() }))}
      />
    </main>
  );
}
