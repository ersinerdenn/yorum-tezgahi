...
type ReviewData = {
  id: string; overallRating: number; title: string; body: string; usageDuration: string | null;
  verifiedPurchase: boolean; receiptUrl: string | null; createdAt: string;
  user: { displayName: string }; metricScores: MetricScoreRow[];
};
...
export default function ReviewItem({ review, metricSchema, isOwner }: { review: ReviewData; metricSchema: Metric[]; isOwner: boolean }) {
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/reviews/${review.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ overallRating, title, body, usageDuration: usageDuration || undefined, metricScores }) });
    setLoading(false);
    ...
    <article className="rounded-2xl border border-line bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-line pt-3 text-xs text-steelLight">
        ...
        <div className="flex items-center gap-3">
          ...
          {isOwner && (<><button onClick={() => setEditing(true)} className="font-medium text-steel hover:text-ink">Düzenle</button><button onClick={handleDelete} disabled={deleting} className="font-medium text-rust hover:text-rust/80 disabled:opacity-50">{deleting ? "Siliniyor…" : "Sil"}</button></>)}
        </div>
      </div>
    </article>
  );
