...
export default function ReviewForm({ productSlug, metricSchema }: { productSlug: string; metricSchema: Metric[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded-full bg-ink px-4 py-2 text-sm font-medium text-white hover:opacity-90 transition-opacity focus-ring">Yorum yaz</button>;

  return (
    <form onSubmit={submit} className="mt-6 space-y-5 rounded-3xl bg-white p-6 card-shadow">
      <div>
      ...
      <div>
        ...
        <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Kısa bir özet" className="mt-1 w-full rounded-xl bg-[#F5F3FF] px-3 py-2 text-sm outline-none focus-ring" />
      </div>
      <div>
        <label className="text-xs font-semibold uppercase tracking-wide text-steel">Deneyimin</label>
        <textarea required minLength={10} value={body} onChange={(e) => setBody(e.target.value)} rows={4} placeholder="Neyi beğendin, neyi beğenmedin?" className="mt-1 w-full rounded-xl bg-[#F5F3FF] px-3 py-2 text-sm outline-none focus-ring" />
      </div>
      <div>
        ...
        <input value={usageDuration} onChange={(e) => setUsageDuration(e.target.value)} placeholder="ör. 3 aydır kullanıyorum" className="mt-1 w-full rounded-xl bg-[#F5F3FF] px-3 py-2 text-sm outline-none focus-ring" />
      </div>
            ...
            {metricSchema.map((m) => (
              <div key={m.key} className="flex items-center justify-between gap-4">
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((n) => (<button type="button" key={n} onClick={() => setMetricScores((s) => ({ ...s, [m.key]: n }))} className={`h-7 w-7 rounded-full text-xs font-medium transition-colors ${metricScores[m.key] === n ? "bg-ink text-white" : "bg-[#F5F3FF] text-steel"}`}>{n}</button>))}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-xl border border-dashed border-line bg-[#FAF7FF] p-4">
      ...
      <div className="flex gap-3">
        <button type="submit" disabled={loading} className="rounded-full bg-ink px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50">{loading ? "Gönderiliyor…" : "Yorumu paylaş"}</button>
