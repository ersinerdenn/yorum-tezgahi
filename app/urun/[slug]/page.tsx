  ...
  return (
    <main className="mx-auto max-w-4xl px-5 pb-24">
      <div className="border-b border-ink py-8">
        <Link href={`/kategori/${product.subcategory.slug}`} className="font-mono text-xs uppercase tracking-widest text-steel hover:text-ink">← {product.subcategory.name}</Link>
      ...
      <section className="border-b border-ink py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {user ? <ReviewForm productSlug={product.slug} metricSchema={metricSchema} /> : (
            <Link href="/giris" className="border border-ink px-4 py-2 text-sm font-medium hover:bg-ink hover:text-paper transition-colors focus-ring">Yorum yazmak için giriş yap</Link>
          )}
        </div>

        {reviewCount === 0 ? (
          <p className="mt-6 border border-dashed border-line bg-white p-6 text-sm text-steel">Bu ürün için henüz yorum yapılmadı. İlk yorumu sen yazabilirsin.</p>
        ) : (
            ...
            {product.reviews.map((r) => (
              <article key={r.id} className="border border-ink bg-white p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {r.verifiedPurchase && <span className="font-mono text-xs text-teal">● Doğrulanmış Alışveriş</span>}
                </div>
                ...
                {r.metricScores.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-4 border-t border-line pt-4">
                    {r.metricScores.map((m) => {
                      ...
                      return <span key={m.id} className="font-mono text-xs text-steel">{label}: <span className="font-semibold text-ink">{m.score}/5</span></span>;
                    })}
                  </div>
                )}
