...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const subcategory = await prisma.subcategory.findUnique({ where: { slug: params.slug } });
  if (!subcategory) return {};
  const title = `${subcategory.name} Yorumları ve Karşılaştırmaları`;
  const description = `${subcategory.name} kategorisindeki ürünler için doğrulanmış kullanıcı yorumları, puanlar ve karşılaştırmalar.`;
  return { title, description, alternates: { canonical: `/kategori/${subcategory.slug}` }, openGraph: { title, description } };
}
      ...
      <section className="py-6">
        {products.length === 0 ? (
          <div className="rounded-2xl bg-white p-6 text-sm text-steel shadow-sm">
            ...
            <Link href={`/urun-ekle?kategori=${subcategory.slug}`} className="mt-3 inline-block rounded-full bg-ink px-4 py-2 text-sm font-medium text-white hover:opacity-90 transition-opacity focus-ring">İlk ürünü sen ekle</Link>
          </div>
        ) : (
