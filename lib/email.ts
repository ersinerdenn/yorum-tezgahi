export async function sendOtpEmail(email: string, code: string) {
  ...
  if (!apiKey) {
    // API anahtarı henüz eklenmediyse, geliştirme sırasında kodu sunucu
    // loglarında görebilelim diye burada yazdırıyoruz.
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family: sans-serif; padding: 24px; background:#EEF0EF;">
          ...
          <p style="color:#3D4552;font-size:13px;">Bu kod 10 dakika içinde geçerliliğini yitirecek. Bu isteği sen yapmadıysan bu e-postayı yok sayabilirsin.</p>
        </div>
      `,
    }),
  });
